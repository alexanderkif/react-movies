...
describe('Movie card test', () => {
  ...
  const setUp = () => shallow(<MovieView {...useMovie(setDialogOpenedHandler, history)} movie={stubMovie1} />);
  ...
  it('click Movie edit', () => {
    ...
    expect(setDialogOpenedHandler.mock.results[0].value).toEqual([true, stubMovie1, false]);
  });

  it('click Movie delete', () => {
    const component = setUp();
    component.find('.delete').simulate('click');
    expect(setDialogOpenedHandler.mock.results[1].value).toEqual([true, stubMovie1, true]);
  });
})
