...
describe('acions test', () => {
  ...
  it('ADD_MOVIES_TO_STORE test', () => {
    expect(
      moviesState({}, getMoviesSuccess([stubMovie1, stubMovie2])).movies,
    ).toStrictEqual([stubMovie1, stubMovie2]);
  });
  ...
  it('getMovies test', async () => {
    ...
    expect(mockedAxios.get).toHaveBeenCalledWith(
      `${MOVIES_URL}?sortBy=vote_average&sortOrder=asc&search=gold&searchBy=title&filter=&limit=50&&offset=0`,
    );
  });
  ...
  it('getMovies error test', async () => {
    ...
    expect(mockedAxios.get).toHaveBeenCalledWith(
      `${MOVIES_URL}?sortBy=vote_average&sortOrder=asc&search=&searchBy=title&filter=&limit=50&&offset=0`,
    );
  });
  ...
  it('getMoviesByGenre error test', async () => {
    ...
    expect(mockedAxios.get).toHaveBeenCalledWith(
      `${MOVIES_URL}?sortBy=release_date&sortOrder=desc&search=&searchBy=genres&limit=25&&offset=0`,
    );
  });
  ...
  it('getMovieById test', async () => {
    ...
    expect(mockedAxios.get).toHaveBeenCalledWith(`${MOVIES_URL}/872`);
  });
  ...
  it('updateMovie error test', async () => {
    ...
    expect(dispatch).toBeCalledTimes(0);
    expect(mockedAxios.put).toHaveBeenCalledWith(`${MOVIES_URL}`, stubMovie2);
  });
  ...
  it('deleteMovieById test', async () => {
    ...
    expect(mockedAxios.delete).toHaveBeenCalledWith(`${MOVIES_URL}/872`);
  });
  ...
  it('GET_MOVIE_BY_ID test', () => {
    expect(
      moviesState({}, getMovieByIdSuccess(stubMovie2)).movie,
    ).toStrictEqual(stubMovie2);
  });
  ...
  it('SET_SEARCH test', () => {
    ...
    expect(moviesState({}, setSearchInput(searchInput)).searchInput).toBe(
      searchInput,
    );
  });
  ...
  it('SET_MOVIES_BY_GENRE test', () => {
    expect(
      moviesState({}, getMoviesByGenreSuccess([stubMovie1, stubMovie2]))
        .moviesByGenre,
    ).toStrictEqual([stubMovie1, stubMovie2]);
  });
  ...
  it('SET_ACTIVE_GENRE_DETAILS test', () => {
    ...
    expect(
      moviesState({}, setActiveGenreDetails(activeGenreDetails))
        .activeGenreDetails,
    ).toBe(activeGenreDetails);
  });

  it('SET_DIALOG_OPEN test', () => {
    const dialogOpened = false;
    expect(
      moviesState({}, setDialogOpened(dialogOpened)).dialogOpened,
    ).toBeFalsy();
  });
});
